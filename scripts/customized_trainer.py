...
class GRPOCustomEvalSaveCallback(CustomEvalSaveCallback):
    ...
    def compute_reward(self, state: TrainerState, metrics):
        return metrics.get("eval_reward", None)

    def compute_kl(self, state: TrainerState, metrics):
        return metrics.get("eval_kl", None)
    ...
    def on_evaluate(
        ...
        # Append eval_loss to file
        eval_loss = self.compute_loss(state, metrics)
        eval_reward = self.compute_reward(state, metrics)
        eval_kl = self.compute_kl(state, metrics)
        if state.global_step < 2:
            return 
        print(f"GO INTO CUSTOMIZED EVALUATE AT STEP: {state.global_step}", flush=True)
        if self.best_checkpoint_info is None or (eval_reward >= self.best_checkpoint_info["reward"] and eval_kl <= self.best_checkpoint_info["kl"]):
            print(f"Updating the best GRPO checkpoint info at step: {state.global_step} with eval_reward: {eval_reward}", flush=True)
            self.best_checkpoint_info = {
                "loss": eval_loss,
                "reward": eval_reward,
                "kl": eval_kl,
            ...
            if self.best_checkpoint_info is not None:
                print(f" At step: {state.global_step} The eval_reward: {eval_reward} is not bigger than the current best eval_reward: {self.best_checkpoint_info['reward']}, update_best_checkpoint={self.update_best_checkpoint}", flush=True)
...
class DPOCustomEvalSaveCallback(CustomEvalSaveCallback):
    ...
    def compute_accuracies(self, state: TrainerState, metrics):
        return metrics.get("eval_rewards/accuracies", None)

    def compute_margins(self, state: TrainerState, metrics):
        return metrics.get("eval_rewards/margins", None)
    ...
    def on_evaluate(
        ...
        # Append eval_loss to file
        eval_loss = self.compute_loss(state, metrics)
        eval_accuracies = self.compute_accuracies(state, metrics)
        eval_margins = self.compute_margins(state, metrics)
        if state.global_step < 2:
            return 
        print(f"GO INTO CUSTOMIZED EVALUATE AT STEP: {state.global_step}", flush=True)
        if self.best_checkpoint_info is None or (eval_accuracies >= self.best_checkpoint_info["accuracies"] and eval_margins >= self.best_checkpoint_info["margins"]):
            print(f"Updating the best DPO checkpoint info at step: {state.global_step} with eval_margins: {eval_margins}", flush=True)
            self.best_checkpoint_info = {
                "loss": eval_loss,
                "accuracies": eval_accuracies,
                "margins": eval_margins,
                "step": state.global_step
            }
            self.update_best_checkpoint = True
        else:
            if self.best_checkpoint_info is not None:
                print(f" At step: {state.global_step} The eval_margins: {eval_margins} is not bigger than the current best eval_margins: {self.best_checkpoint_info['margins']}, update_best_checkpoint={self.update_best_checkpoint}", flush=True)
